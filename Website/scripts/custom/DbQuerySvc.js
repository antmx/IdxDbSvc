...
var DbQuerySvc = (function () {
    ...
    var _queryIndexedDb = function (dbName/*: string*/, dbTableName/*: string*/, indexName/*: string*/, keyRange/*: any*/, callBackFunction/*: Function*/, callBackArgs/*: any*/)/*: void*/ {

        var openDbRequest/*: IDBOpenDBRequest*/ = window.indexedDB.open(dbName);
    ...
    /**
     * QueryDeferred Performs a data query. Returns a deferred which is resolved when the async operation completes.
     ...
     * @param {function|string|array<string>} [orderBy] Optional either a function to sort items, a string naming 1 field to sort by, or an array of strings naming multiple fields to sort by.
    ...
    /**
     * QueryInnerJoinDeferred Performs a data query, then joins the results to another array and returns database items that have a matching item in the array. Returns a deferred which is resolved when the async operation completes.
     * @param {string} dbName IndexedDB database name.
     * @param {string} dbTableName IndexedDB store name to query.
     * @param {function} [filterFn] Optional function to filter items in the store being queried.
    ...
    /**
     * QueryLeftJoinDeferred Performs a data query, then left-joins the results to another array and returns all database items regardless of them having a matching item in the array.
     * @param {string} dbName IndexedDB database name.
     * @param {string} dbTableName IndexedDB store name to query.
     * @param {function} filterFn Optional function to filter items in the store being queried.
    ...
    /**
     * StoreDeferred Saves data to the given object store.
        ...
        openDbRequest.onsuccess = function (evt) {
            ...
            try {
                ...
                tx.onerror = function (evt) {
                    ...
                    console.error("DbQuerySvc.StoreDeferred transaction abort at " + storeName + "." + keyPath + ": " + error);

                    alert("Tx error in StoreDeferred for table " + dbTable + " " + error); //Temporary alert until sort out what to do with error here

                    deferred.reject("StoreDeferred tx error"); // Signal error
                };
    ...
    ///**
    // * StoreManyDeferred Saves data to multiple object stores.
    // * @param {string} dbName The IndexedDB database name.
    ...
    //        if (nextItem[1] !== undefined && (!$.isArray(nextItem[1]) || nextItem[1].length > 0)) {
    ...
    /**
     * DeleteDeferred Deletes from given store the rows whose primary-key index value matches one of the specified values. Returns a deferred which handles the async operation.
     * @param {string} dbName The IndexedDB database name.
     * @param {string} dbTable The IndexedDB store/table name.
